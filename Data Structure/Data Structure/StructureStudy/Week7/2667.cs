...
namespace Data_Structure
{
    ...
    class _2667
    {
        ...
        static private bool[,] visited = new bool[26,26];
        static private int[,] graph = new int[26,26]; // 리스트의 리스트로 인접 리스트를 구현.
        static int[,] dir = new int[4,2] { {-1,0 }, { 0,1 }, { 1,0  }, { 0,-1 } };
        static int cnt;
        static int n;
        ...
        static private void dfs(int x, int y)
        {
            for (int i = 0; i < 4; i++) // 네 방향에 따라 확인 (이는 기존에 노드에 연결되어있는 대상 여부를 확인하는것과 같다.)
            {
                ...
                if(nx >= 0 && ny >= 0 && nx <= n && ny <= n) // 만약 nx,ny가 좌표 내에 있다면 
                {
        ...
        public void Solve()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for(int j = 0; j < n; j++)
                {
                    graph[i,j] = (int.Parse(a[j].ToString()));
                }
            }
